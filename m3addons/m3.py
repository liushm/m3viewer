...
class ReferenceField(Field):
    ...
    def getListContentStructureDefinition(self, l, contextString):
        ...
        if not isinstance(firstElement, M3Structure):
...
class CharReferenceField(ReferenceField):
    ...
    def validateContent(self, fieldContent, fieldPath):
        if (fieldContent != None) and not isinstance(fieldContent, (str, unicode)):
...
class IntField(PrimitiveField):
    ...
    def validateContent(self, fieldContent, fieldPath):
        if not isinstance(fieldContent, (int, long)):
...
class ExpectedAndDefaultConstantsDeterminer(Visitor):
    def parseHex(self, hexString):
        ...
        return ''.join([chr(int(hexString[x:x+2], 16)) for x in range(0, len(hexString),2)])
        #TODO how to handle it with both 2 & 3 supporting
